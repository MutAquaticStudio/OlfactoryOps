...
describe('Formula Intelligence Worker persistence contract', () => {
  it('requires tenant-scoped compliance approval for every material candidate', () => {
    const pending = { id: 'mat-pending', name: 'Pending material' }
    const reviewed = { id: 'mat-reviewed', name: 'Bergamot FCF' }
    const service = {
      materials: () => ({ data: [pending, reviewed] }),
      materialCompliance: (id: string) => ({ data: id === 'mat-reviewed' ? { status: 'APPROVED' } : undefined }),
