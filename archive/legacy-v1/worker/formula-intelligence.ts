...
async function runDesignStudio(
  ...
  const evidenceMaterialIds = materials.map((material) => material.id).slice(0, 12)
  ...
  await store.createAssistantMessage(run, modelProvider.kind === 'workers_ai'
    ? 'Cloudflare Workers AI interpreted the creative brief with governed workspace evidence retrieval. Three directions were calculated only from reviewed workspace materials, inventory visibility, and compliance gates.'
    : 'I prepared three deterministic directions from reviewed workspace materials. A perfumer can share a direction for brand review or explicitly save one as a draft.')
}
