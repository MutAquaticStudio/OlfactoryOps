...
import { candidateAcceptanceConfig, candidateBrowserConfig, safeExecutionFailure, verifyProductionCandidateAcceptance } from './verify-v2-production-candidate-acceptance.mjs'
...
const validEnvironment = {
  ...
  V2_PRODUCTION_CANDIDATE_EXPECTED_SHA: 'a'.repeat(40),
...
test('candidate acceptance config permits only the fixed isolated origins', () => {
  ...
  expect(config.expectedSha).toBe('a'.repeat(40))
})
...
test('candidate Pages CSP explicitly permits only the candidate API origin', () => {
  ...
  expect(headers).toContain('https://api-next.labofscents.org')
  expect(headers).not.toContain('https://*.next.labofscents.org')
})
