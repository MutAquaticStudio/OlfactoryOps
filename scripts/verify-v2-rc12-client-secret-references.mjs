...
export function verifyClientSecretReferences(root) {
  if (!isAbsolute(root) || !existsSync(root)) throw new Error('RC12_CLIENT_SOURCE_ROOT_INVALID')
  const findings = inspectClientReferences(trackedClientEntries(root))
  if (findings.length > 0) throw new Error('RC12_CLIENT_SECRET_REFERENCE_FOUND')
