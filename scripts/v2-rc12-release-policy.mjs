...
export const RC12_TAG = 'v2-production-rc12'
...
function runtimePath(path) {
  return path === 'package.json'
    ...
    || /^(?:src|worker|server|services)\//.test(path)
}

export function parseRc12Manifest(serialized) {
  try {
...
export function validateRc12Manifest(manifest) {
  ...
  if (release.sourceBranch !== 'main' || release.sourceSha !== RC12_SHA || release.sourceTag !== RC12_TAG || release.previousReleaseTag !== 'v2-production-rc11' || release.previousReleaseSha !== RC11_SHA || !shaPattern.test(release.sourceSha) || !shaPattern.test(release.previousReleaseSha)) return { pass: false, state: 'RELEASE_IDENTITY_INVALID' }
  if (!sameObject(manifest.invariants, expectedInvariants)) return { pass: false, state: 'RELEASE_INVARIANTS_INVALID' }
  if (!manifest.includedPullRequests.every(Number.isSafeInteger) || new Set(manifest.includedPullRequests).size !== manifest.includedPullRequests.length || manifest.includedPullRequests.some((value, index, values) => value <= 0 || (index > 0 && values[index - 1] >= value))) return { pass: false, state: 'PULL_REQUEST_MANIFEST_INVALID' }
  const paths = new Set()
  ...
  if (unrelatedRuntimeChangeCount !== 0) return { pass: false, state: 'UNRELATED_RUNTIME_CHANGE_DETECTED' }
  return { pass: true, state: 'READY' }
}

export function verifyRc12Source({ manifest, sourceSha, previousSha, changedPaths, fileContents }) {
  const validation = validateRc12Manifest(manifest)
  if (!validation.pass) return validation
  if (sourceSha !== RC12_SHA || previousSha !== RC11_SHA) return { pass: false, state: 'SOURCE_REVISION_INVALID' }
...
export function verifyRc12SourceCheckout({ manifest, repository, tagRequired = true }) {
  const sourceSha = git(repository, ['rev-parse', 'HEAD'])
  const previousSha = git(repository, ['rev-parse', 'v2-production-rc11^{commit}'])
  const tagSha = git(repository, ['rev-parse', 'v2-production-rc12^{commit}'])
  const changed = git(repository, ['diff', '--name-only', `${RC11_SHA}..${RC12_SHA}`])
  if (!sourceSha || !previousSha || changed === null) return { pass: false, state: 'SOURCE_READ_UNAVAILABLE' }
  if ((tagRequired && !tagSha) || (tagSha && tagSha !== RC12_SHA)) return { pass: false, state: 'SOURCE_TAG_INVALID' }
  const fileContents = Object.fromEntries(manifest.changedFiles.map(({ path }) => [path, git(repository, ['show', `${RC12_SHA}:${path}`], { encoding: null, trim: false })]))
  return verifyRc12Source({ manifest, sourceSha, previousSha, changedPaths: changed ? changed.split('\n').filter(Boolean) : [], fileContents })
}

export function runRc12ReleasePolicy({ environment = process.env, readFile = readFileSync, emit = (line) => console.log(line) } = {}) {
  const path = environment.RC12_RELEASE_MANIFEST_PATH?.trim()
  const repository = environment.RC12_SOURCE_WORKTREE?.trim()
  const tagRequired = environment.RC12_SOURCE_TAG_REQUIRED?.trim() !== 'false'
  let result = { pass: false, state: 'MANIFEST_UNAVAILABLE' }
  if (path && repository) {
    try {
      const parsed = parseRc12Manifest(readFile(path, 'utf8'))
      result = parsed.pass ? verifyRc12SourceCheckout({ manifest: parsed.manifest, repository, tagRequired }) : parsed
    } catch {
      result = { pass: false, state: 'MANIFEST_UNAVAILABLE' }
    }
  }
  emit(`RC12_RELEASE_MANIFEST=${result.pass ? 'PASS' : 'FAIL'}`)
  emit(`RC12_SOURCE_SCOPE=${result.pass ? 'PASS' : 'FAIL'}`)
  emit(`UNRELATED_RUNTIME_CHANGE_COUNT=${result.pass ? '0' : 'UNPROVEN'}`)
  emit(`RC12_SOURCE_STATE=${result.state}`)
  return result
}

if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  const result = runRc12ReleasePolicy()
