...
} from "./capture-v2-rc11-upgrade-state.mjs";
...
it("requires exact release binding and safe production Pages identity", () => {
  ...
  const pages = inspectPagesProductionDeployment(
    { success: true, result: [{ id: "opaque_pages_deployment", environment: "production", latest_stage: { name: "success" }, deployment_trigger: { metadata: { commit_hash: RC11_SHA } } }] },
    RC11_SHA,
  );
  expect(pages.pass).toBe(true);
  assert.equal(
    inspectPagesProductionDeployment({ success: true, result: [] }, RC11_SHA).state,
    "PRODUCTION_DEPLOYMENT_UNPROVEN",
  );
});

it("selects a unique tagged inactive upload and does not serialize provider sentinels", () => {
  const result = inspectUploadedVersion([{ id: activeId, tag: `rc11-${RC11_SHA.slice(0, 12)}` }]);
