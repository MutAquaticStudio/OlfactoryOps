...
describe("router request-path tail verifier", () => {
  it("extracts only the expected nonce event into safe classifications", () => {
    ...
    expect(output).toContain("TAIL_READINESS=PASS");
    expect(output).toContain("TAIL_VERSION_FILTER_APPLIED=PASS");
  ...
  it("classifies unavailable Tail permission without printing its raw error", () => {
    ...
    expect(output).toContain("TAIL_READINESS=UNAVAILABLE");
    expect(output).toContain("TAIL_VERSION_FILTER_APPLIED=UNPROVEN");
  ...
  it("does not claim a version-filtered tail when startup fails", () => {
    const output = run("", "Error: failed to start stream");
    expect(output).toContain("TAIL_PERMISSION_AVAILABLE=YES");
    expect(output).toContain("TAIL_READINESS=FAIL");
    expect(output).toContain("TAIL_VERSION_FILTER_APPLIED=UNPROVEN");
    expect(output).not.toContain("failed to start stream");
  });
