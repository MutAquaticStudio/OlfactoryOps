...
} from "./verify-v2-rc13-candidate-pages-project-root.mjs";
...
const releaseSha = "09e96feacb9db03325683ee329fb269206a21880";
...
async function withDist(action, { js = javascript, css = stylesheet } = {}) {
  const directory = mkdtempSync(join(tmpdir(), "oo-rc13-pages-root-"));
  try {
...
describe("RC13 candidate Pages project-root verifier", () => {
  it("proves the isolated project, exact canonical RC13 deployment, and byte-identical runtime artifact", async () => {
    await withDist(async (distDirectory) => {
      ...
      expect(output).toEqual([
        ...
        "CANDIDATE_PROJECT_ROOT_RELEASE_SHA=RC13",
