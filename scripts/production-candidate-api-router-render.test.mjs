...
function runRenderer(directory, renderer, environment) {
  return execFileSync(process.execPath, ['--input-type=module', '--eval', renderer], {
    ...
    env: {
      ...
      CANDIDATE_TENANT_ROUTE: candidateTenantRoute,
      ...environment,
    },
  })
}
...
test('candidate tenant router renderer keeps its config at root and resolves the immutable RC2 entrypoint', () => {
  withWorkspace((directory) => {
    ...
    expect(config).toContain(candidateTenantRoute)
...
test('candidate namespace is valid, coherent, and cannot target public production routes', () => {
  ...
  expect(routerJob).toContain('process.env.CANDIDATE_TENANT_ROUTE')
  ...
  expect(workflow).toContain('CANDIDATE_TENANT_ROUTE: "*.next.labofscents.org/*"')
