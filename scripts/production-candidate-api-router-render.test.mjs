...
function runRenderer(directory, renderer, environment) {
  return execFileSync(process.execPath, ['--input-type=module', '--eval', renderer], {
    ...
    encoding: 'utf8',
    stdio: 'pipe',
    env: { ...process.env, RELEASE_SHA: immutableRc2Sha, PRODUCTION_HYPERDRIVE_ID: hyperdriveId, ...environment },
  })
}
...
test('candidate tenant router renderer keeps its config at root and resolves the immutable RC2 entrypoint', () => {
  withWorkspace((directory) => {
    ...
    expect(config).toContain('workspace-*-next.labofscents.org/*')
    expect(config).not.toContain('*.labofscents.org/*')
    expect(config).not.toContain('REPLACE_WITH_')
  })
})

test('API and tenant router candidate deployments are root-configured and do not target public production routes', () => {
  ...
  expect(routerJob).toContain('workspace-*-next.labofscents.org/*')
  expect(routerJob).not.toMatch(/routes\s*=\s*\[\{ pattern = "\*\.labofscents\.org\/\*"/)
})
