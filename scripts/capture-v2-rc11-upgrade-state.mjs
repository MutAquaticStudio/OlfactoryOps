...
export const RC11_VERSION_TAG = `rc11-${RC11_SHA.slice(0, 12)}`;
...
export function inspectUploadedVersion(list, tag = RC11_VERSION_TAG) {
...
function stateDirectory(environment = process.env) {
  const directory = environment.RC11_UPGRADE_STATE_DIRECTORY?.trim();
...
function readState(directory) {
  try {
    ...
    if (!state || state.rc10Sha !== RC10_SHA || state.rc11Sha !== RC11_SHA) throw new Error();
    return state;
  } catch {
...
async function capture(environment = process.env) {
  ...
  writeState(directory, { rc10Sha: RC10_SHA, rc11Sha: RC11_SHA, workers, pages: { deploymentId: production.deploymentId } });
...
function uploaded(environment = process.env) {
  ...
  console.log("RC11_INACTIVE_VERSION_UPLOADS=PASS");
}
...
async function verifyPromoted(key, environment = process.env) {
  ...
  if (!detail.ok || !inspectVersionIdentity(detail.body, versionId, RC11_SHA).pass) throw new Error("PROMOTION_IDENTITY_UNVERIFIED");
  console.log(`RC11_${key.toUpperCase().replaceAll("-", "_")}_PROMOTION=PASS`);
}
...
async function verifyPages(environment = process.env) {
  ...
  if (!pages.ok || !inspectPagesProductionDeployment(pages.body, RC11_SHA).pass) throw new Error("RC11_PAGES_PROMOTION_UNVERIFIED");
  console.log("RC11_PAGES_PRODUCTION_IDENTITY=PASS");
}
...
async function verifyLive(environment = process.env) {
  for (const [key, service] of Object.entries(SERVICES)) {
    ...
    const active = deployment.ok ? inspectActiveDeployment(deployment.body) : { pass: false };
    if (!active.pass) throw new Error("RC11_ACTIVE_DEPLOYMENT_UNPROVEN");
    const detail = await request(
      `/accounts/${encodeURIComponent(environment.CLOUDFLARE_ACCOUNT_ID ?? "")}/workers/scripts/${encodeURIComponent(service)}/versions/${encodeURIComponent(active.versionId)}`,
      { environment },
    );
    if (!detail.ok || !inspectVersionIdentity(detail.body, active.versionId, RC11_SHA).pass) {
      throw new Error("RC11_ACTIVE_IDENTITY_UNPROVEN");
    }
    console.log(`RC11_${key.toUpperCase().replaceAll("-", "_")}_ACTIVE=PASS`);
  }
  await verifyPages(environment);
  console.log("RC11_ACTIVE_COMPONENTS=PASS");
}
...
async function verifyRollbackCapability(environment = process.env) {
  ...
  console.log("RC11_UPGRADE_ROLLBACK_CAPABILITY=PASS");
}
async function main() {
  ...
  try {
    ...
    else if (mode === "promote" || mode === "assert-rc10-rollback-targets" || mode === "verify-rc10-public") console.log("RC11_UPGRADE_STATE=PASS");
    else throw new Error("INVALID_MODE");
  } catch {
    console.log("RC11_UPGRADE_STATE=FAIL");
    process.exitCode = 1;
  }
}
