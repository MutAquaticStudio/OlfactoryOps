...
import { RC13_SHA, candidateAcceptanceConfig, candidateBrowserConfig, generatedLoginWorkspaceRedirectMatches, generatedWorkspaceRedirectMatches, safeExecutionFailure, verifyProductionCandidateAcceptance } from './verify-v2-rc13-production-candidate-acceptance.mjs'
...
const validEnvironment = {
  ...
  V2_PRODUCTION_CANDIDATE_EXPECTED_SHA: RC13_SHA,
...
test('candidate acceptance config permits only the fixed isolated origins', () => {
  ...
  expect(config.expectedSha).toBe(RC13_SHA)
})
...
test('candidate acceptance rejects arbitrary API, tenant, database, and revision targets', () => {
  for (const changes of [
    ...
    { V2_PRODUCTION_CANDIDATE_EXPECTED_SHA: '331c1a6054fe1420b063a2e1fe9e5cef4f043ff8' },
  ]) {
...
test('candidate acceptance preserves an existing Platform Owner and never creates one', () => {
  const verifier = readFileSync('scripts/verify-v2-rc13-production-candidate-acceptance.mjs', 'utf8')
...
test('generated candidate signup and login must return the exact RC13 first-party workspace path', () => {
