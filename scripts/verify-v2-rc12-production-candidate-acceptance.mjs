...
export function candidateAcceptanceConfig(environment = process.env, { requireDatabase = true } = {}) {
  ...
  if (expectedSha !== RC12_SHA) blocked('EXPECTED_SHA_INVALID')
...
export async function verifyProductionCandidateAcceptance(environment = process.env) {
  ...
  const roleResults = []
  let signupWorkspaceRedirect = false
  let loginWorkspaceRedirect = false
  ...
  async function signup(label, input) {
    ...
    assert(generatedWorkspaceRedirectMatches(hostname, result.body?.workspaceUrl, config.workspaceBaseDomain), 'signup_workspace_redirect_invalid')
    signupWorkspaceRedirect = true
  ...
  assert(signupWorkspaceRedirect, 'signup_workspace_redirect_not_proven')
  assert(loginWorkspaceRedirect, 'login_workspace_redirect_not_proven')
