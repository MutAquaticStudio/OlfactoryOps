...
const backup = readFileSync('.github/workflows/v2-rc10-production-backup-snapshot.yml', 'utf8')
const pages = readFileSync('.github/workflows/v2-rc10-production-pages-project-preparation.yml', 'utf8')
const readiness = readFileSync('.github/workflows/v2-rc10-production-readiness.yml', 'utf8')
const dispatcher = readFileSync('.github/workflows/v2-production-release-dispatch.yml', 'utf8')
const acceptance = readFileSync('.github/workflows/v2-production-public-acceptance.yml', 'utf8')
const rollback = readFileSync('scripts/verify-v2-production-rollback-readiness.mjs', 'utf8')
const publicHarness = readFileSync('scripts/verify-v2-production-public-acceptance.mjs', 'utf8')
...
describe('RC10 final readiness operations', () => {
  it('creates and validates a private custom-format backup only', () => {
    ...
    expect(backup).toContain('/r2/buckets/')
  ...
  it('requires immutable backup evidence and smoke-tenant proof', () => {
    ...
    expect(readiness).toContain('test "$((now_epoch - created_epoch))" -le 86400')
    expect(readiness).toContain('resolve-v2-production-pages-project.mjs')
    expect(readiness).toContain('verify-v2-production-smoke-tenant-readiness.mjs')
  ...
  it('resolves production Pages before either Pages or Router mutation', () => {
    ...
    expect(dispatcher).not.toContain('PRODUCTION_PAGES_PROJECT: ${{ vars.PRODUCTION_PAGES_PROJECT }}')
