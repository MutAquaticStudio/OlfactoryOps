...
export function emitRc12InactiveUploadFailure(evidence, emit = console.log) {
  emit(`RC12_INACTIVE_UPLOAD_COMPONENT=${evidence.component}`);
  emit(`RC12_INACTIVE_UPLOAD_HTTP_STATUS=${evidence.httpStatus}`);
  emit(`RC12_INACTIVE_UPLOAD_CF_ERROR_CODE=${evidence.cloudflareErrorCode}`);
  emit(`RC12_INACTIVE_UPLOAD_FAILURE_CLASS=${evidence.failureClass}`);
}
