...
import { DesignStudioGlobalMaterialReference } from './DesignStudioGlobalMaterialReference'
...
const GlobalMaterialIntelligenceWorkspace = lazy(async () => ({ default: (await import('./GlobalMaterialIntelligenceWorkspace')).GlobalMaterialIntelligenceWorkspace }))
...
function workspaceSection() {
  ...
  if (pathname === '/material-intelligence' || pathname.startsWith('/material-intelligence/') || pathname === '/v2/material-intelligence' || pathname.startsWith('/v2/material-intelligence/') || pathname === '/v2/workspace/material-intelligence' || pathname.startsWith('/v2/workspace/material-intelligence/')) {
...
export const workspaceNavigation: readonly WorkspaceNavigationGroup[] = [
  { label: 'Home', items: [{ key: 'workspace', label: 'Workspace overview', permissions: ['tenant.view'], icon: Home }] },
  {
    ...
    items: [
      ...
      { key: 'material-intelligence', label: 'Material Intelligence', permissions: ['materials.view'], icon: Atom },
...
const workspaceTitles: Record<string, { eyebrow: string; title: string; description: string }> = {
  ...
  'material-intelligence': { eyebrow: 'Global reference', title: 'Material Intelligence', description: 'A platform-wide, read-only scientific catalog with source-aware identity, properties, taxonomy, and provenance.' },
...
function WorkspaceView({ text, locale, onLocale, onNavigate }: { text: PlatformCopy; locale: Locale; onLocale: () => void; onNavigate: (path: string) => void }) {
  ...
  const selectItem = (key: string) => {
    ...
    setCommandQuery('')
    onNavigate(key === 'material-intelligence' ? '/material-intelligence' : `/v2/workspace${key === 'workspace' ? '' : `/${key}`}`)
  }
    ...
    : active === 'design-studio' ? ['Briefs are reviewed before a material universe is pinned.', 'No external provider is assumed.', 'Creative recommendations remain a human decision.']
      : active === 'agents' ? ['Runs, approvals, and evidence are persisted.', 'Tool execution is server-authoritative.', 'Private reasoning is never displayed.']
        : active === 'materials' ? ['This route shows tenant material records.', 'Supplier and global catalog records are not fabricated.', 'Operational states are retained as recorded.']
          : active === 'material-intelligence' ? ['This catalog is global and read only.', 'Tenant materials and working preparations remain separate.', 'Every claim retains versioned source provenance.']
...
function V2Section({ active, text, locale, session, onNavigate }: { active: string; text: PlatformCopy; locale: Locale; session: V2Session | null; onNavigate: (path: string) => void }) {
  ...
  if (active === 'material-intelligence') return <Suspense fallback={<WorkspaceSurfaceFallback />}><GlobalMaterialIntelligenceWorkspace apiBase={materialIntelligenceApiBase} onNavigate={onNavigate} /></Suspense>
  ...
  if (active === 'formulas' || active === 'design-studio') return <FormulaIntelligencePanel active={active} globalMaterialApiBase={materialIntelligenceApiBase} />
...
function FormulaIntelligencePanel({ active, globalMaterialApiBase }: { active: 'formulas' | 'design-studio'; globalMaterialApiBase: string }) {
