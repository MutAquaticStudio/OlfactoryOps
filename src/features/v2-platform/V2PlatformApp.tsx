...
function WorkspaceView({ text, locale, onLocale, onNavigate }: { text: PlatformCopy; locale: Locale; onLocale: () => void; onNavigate: (path: string) => void }) {
  ...
  useEffect(() => {
    void request<{ csrfToken: string }>('/auth/csrf/bootstrap', { method: 'POST', body: '{}' })
      ...
      .then(setSession)
      .catch((err) => { setError(err instanceof Error ? err.message : text.unavailable); onNavigate('/login') })
      .finally(() => setBusy(false))
  }, [onNavigate, text.unavailable])
  const items = useMemo(() => [
    ...
    { key: 'advanced', label: 'Optimizer & DataOps', permissions: ['optimizer.view', 'imports.view', 'dataops.view', 'bulk.preview'] },
