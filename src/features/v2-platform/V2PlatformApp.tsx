...
function isWorkspaceFeatureAvailableInPublicCutover(key: string, publicCutover: boolean) {
  return !publicCutover || !stagingExcludedSections.has(key)
}
...
function LabOperationsPanel({ active, capabilities }: { active: 'materials' | 'suppliers' | 'inventory' | 'procurement'; capabilities: Record<string, boolean> }) {
  ...
  if (active === 'materials') {
    ...
    return <div className="v2-panel v2-material-library" data-testid="v2-materials">
      ...
      <div className="v2-data-table-wrap" tabIndex={0}><table className="v2-data-table"><thead><tr><th scope="col">Material</th><th scope="col">CAS</th><th scope="col">Olfactive family</th><th scope="col">Source</th><th scope="col">Availability</th><th scope="col">Compliance</th><th scope="col">Cost</th></tr></thead><tbody>{visibleMaterials.length ? visibleMaterials.map((material) => <tr key={material.id}><td><button type="button" className="v2-table-primary" onClick={() => setNotice(`${material.name} is selected in the tenant material library.`)}>{material.name}<small className="v2-mono">{material.internalCode || 'No internal code'}</small></button></td><td>Not captured</td><td>Not captured</td><td><span className="v2-scope-chip is-active">Tenant</span></td><td><span className={`v2-status-chip status-${material.status.toLowerCase()}`}>{material.status.replaceAll('_', ' ')}</span></td><td>Not captured</td><td>Not captured</td></tr>) : <tr><td colSpan={7}><div className="v2-table-empty">{materials.length ? 'No material record matches the current filters.' : 'No tenant materials have been created yet.'}</div></td></tr>}</tbody></table></div>
