...
function WorkspaceDialog({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  ...
  return <div className="formula-intelligence-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="formula-intelligence-modal" role="dialog" aria-modal="true" aria-label={title}><div className="panel-title-row"><h3>{title}</h3><button ref={closeRef} className="icon-button" type="button" onClick={onClose} aria-label="Close dialog"><X size={17} /></button></div>{children}</section></div>
}
...
function RunStatus({ detail, connectionState, busy, onCancel }: { detail?: RunDetail; connectionState: ConnectionState; busy: boolean; onCancel: () => void }) {
  ...
  return <section className="formula-intelligence-run-status" aria-live="polite"><div><strong>{runLabel(detail.run.status)}</strong><span>{connectionState === 'live' ? 'Live updates' : connectionState === 'reconnecting' ? 'Reconnecting to durable run' : 'Restored from durable run'}</span></div><div className="agent-progress"><span style={{ width: `${detail.run.progress}%` }} /></div>{detail.nodes?.length ? <div className="formula-intelligence-node-list">{detail.nodes.map((node) => <span key={node.id} className={`is-${node.status.toLowerCase()}`}>{node.node_type.replaceAll('_', ' ')}: {node.status.toLowerCase()}</span>)}</div> : null}{detail.run.status === 'RUNNING' || detail.run.status === 'QUEUED' ? <button className="ghost-button small" type="button" disabled={busy} onClick={onCancel}>Cancel run</button> : null}</section>
}
...
export function FormulaDesignStudioWorkspace({ apiBaseUrl, requestApi, materialRecords, capabilities, onFormulaSaved }: { apiBaseUrl: string; requestApi: ApiRequest; materialRecords: Material[]; capabilities: FormulaIntelligenceCapabilities; onFormulaSaved: (formula: Formula) => void }) {
  ...
  return <div className="domain-page formula-intelligence-page">
    ...
    {shareTarget ? <WorkspaceDialog title="Share direction" onClose={() => setShareTarget(undefined)}><p className="formula-intelligence-copy">Only active members of this brand can receive this direction. Material names stay hidden unless you opt in.</p>{shareTarget.direction.shares?.length ? <div className="formula-intelligence-share-list">{shareTarget.direction.shares.map((share) => <div key={share.recipientUserId}><span>{shareRecipients.find((recipient) => recipient.userId === share.recipientUserId)?.name ?? 'Active recipient'}{share.allowMaterialNames ? ' / material names visible' : ''}</span><button className="ghost-button small" type="button" disabled={busy} onClick={() => void revokeShare(shareTarget.projectId, shareTarget.direction.directionId, share.recipientUserId)}>Revoke</button></div>)}</div> : null}<div className="formula-intelligence-recipient-list">{shareRecipients.map((recipient) => <label key={recipient.userId}><input type="checkbox" checked={selectedRecipientIds.includes(recipient.userId)} onChange={() => setSelectedRecipientIds((current) => current.includes(recipient.userId) ? current.filter((id) => id !== recipient.userId) : [...current, recipient.userId])} /><span><strong>{recipient.name}</strong><small>{recipient.email}</small></span></label>)}</div><label className="checkbox-row"><input type="checkbox" checked={allowMaterialNames} onChange={(event) => setAllowMaterialNames(event.target.checked)} /> Disclose material names</label><div className="formula-intelligence-actions"><button className="primary-button small" type="button" disabled={busy || selectedRecipientIds.length === 0} onClick={() => void share()}><Share2 size={14} /> Save sharing</button><button className="secondary-button small" type="button" disabled={busy} onClick={() => setShareTarget(undefined)}>Cancel</button></div></WorkspaceDialog> : null}
    {pending ? <WorkspaceDialog title="Confirm formula draft" onClose={() => setPending(undefined)}><p className="formula-intelligence-copy">{pending.label}</p><p className="formula-intelligence-copy">This creates one editable draft. It does not reserve or consume inventory.</p><div className="formula-intelligence-actions"><button className="primary-button" type="button" disabled={busy} onClick={() => void confirmSave()}><CheckCircle2 size={16} /> Confirm draft</button><button className="secondary-button" type="button" disabled={busy} onClick={() => setPending(undefined)}>Not now</button></div></WorkspaceDialog> : null}
  </div>
}
