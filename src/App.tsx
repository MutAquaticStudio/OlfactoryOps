...
import { browserWorkspaceBaseDomain } from './features/v2-platform/workspaceHostnameRuntime'
...
function LegacyApp() {
  ...
  const navigatePublic = useCallback((path: '/login' | '/signup', replace = false) => {
    const publicAuthUrl = publicAuthUrlForWorkspaceOrigin(path, window.location.origin, browserWorkspaceBaseDomain)
    if (publicAuthUrl) {
