...
export class PrismaPlatformRepository implements PlatformRepository {
  ...
  async transaction<T>(callback: (repository: PlatformRepository) => Promise<T>, context?: RepositoryContext): Promise<T> {
    return this.client.$transaction(async (tx): Promise<T> => {
      ...
      if (context?.passwordResetHash) await tx.$executeRaw`SELECT set_config('app.password_reset_hash', ${context.passwordResetHash}, true)`
  ...
  async revokeAllUserSessions(userId: string, reason: string) { await this.client.session.updateMany({ where: { userId, revokedAt: null }, data: { revokedAt: new Date(), revokeReason: reason } }) }
  ...
  async savePasswordReset(record: PasswordResetRecord) { await this.client.passwordReset.create({ data: { id: record.id, userId: record.userId, organizationId: record.organizationId, tokenHash: record.tokenHash, expiresAt: new Date(record.expiresAt), createdAt: new Date(record.createdAt) } }) }
  async findLatestPasswordReset(userId: string, organizationId: string) { const row = await this.client.passwordReset.findFirst({ where: { userId, organizationId }, orderBy: { createdAt: 'desc' } }); return row ? this.passwordReset(row) : null }
  async findPasswordReset(tokenHash: string) { const run = async (client: PrismaClient) => { await client.$executeRawUnsafe(`SELECT set_config('app.password_reset_hash', $1, true)`, tokenHash); return client.passwordReset.findUnique({ where: { tokenHash } }) }; const row = typeof (this.client as unknown as { $transaction?: unknown }).$transaction === 'function' ? await this.client.$transaction((tx) => run(tx as unknown as PrismaClient)) : await run(this.client); return row ? this.passwordReset(row) : null }
  async revokePasswordResets(userId: string) { await this.client.passwordReset.updateMany({ where: { userId, usedAt: null, revokedAt: null }, data: { revokedAt: new Date() } }) }
  async markPasswordResetUsed(id: string, userId: string, tokenHash: string, usedAt: string) { const result = await this.client.passwordReset.updateMany({ where: { id, userId, tokenHash, usedAt: null, revokedAt: null, expiresAt: { gt: new Date(usedAt) } }, data: { usedAt: new Date(usedAt) } }); return result.count === 1 }
  ...
  private passwordReset(row: { id: string; userId: string; organizationId: string; tokenHash: string; expiresAt: Date; createdAt: Date; usedAt: Date | null; revokedAt: Date | null }): PasswordResetRecord { return { id: row.id, userId: row.userId, organizationId: row.organizationId, tokenHash: row.tokenHash, expiresAt: iso(row.expiresAt), createdAt: iso(row.createdAt), usedAt: row.usedAt?.toISOString(), revokedAt: row.revokedAt?.toISOString() } }
