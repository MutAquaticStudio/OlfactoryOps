...
function createNodePostgresRuntimeProbeExecutor(env, hostname) {
  const client = new import_pg.Client({
    connectionString: env.HYPERDRIVE.connectionString
  });
  let connected = false;
  async function queryRow(text, values = []) {
    return firstRow((await client.query(text, values)).rows);
  }
  __name(queryRow, "queryRow");
  return {
...
__name(createNodePostgresRuntimeProbeExecutor, "createNodePostgresRuntimeProbeExecutor");
function emptyDiagnostic(targetReleaseSha) {
  return {
    targetReleaseSha,
...
__name(emptyDiagnostic, "emptyDiagnostic");
async function applyProbe(run, apply) {
  try {
    apply(await run());
  } catch {
  }
}
__name(applyProbe, "applyProbe");
async function inspectCandidateRuntime(env, createExecutor = createNodePostgresRuntimeProbeExecutor) {
...
function runtimeDiagnosticExecutionPass(diagnostic) {
  return diagnostic.databaseProbeCompleted && diagnostic.hyperdriveConnectionReachable && diagnostic.hyperdriveProductionDatabaseMatch && diagnostic.runtimeCurrentUserMatchesExpected && diagnostic.runtimeSessionUserMatchesExpected;
}
...
function createCandidateRuntimeDiagnostic(inspector = inspectCandidateRuntime) {
  return {
    async fetch(request, env) {
      if (request.method !== "GET" || !env.CANDIDATE_RUNTIME_DIAGNOSTIC_TOKEN || !equalSecret(
