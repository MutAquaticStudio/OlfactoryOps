...
const expectedDatabaseSha = 'a942b37ccfaf5a813b1432caa209a43b9d144e47ad0de1549c289c253e556cd5'
...
function executor(overrides: Partial<RuntimeDiagnosticProbeExecutor> = {}): RuntimeDiagnosticProbeExecutor {
  return {
    databaseIdentity: async () => ({ databaseName: 'postgres', currentUserMatchesExpected: true, sessionUserMatchesExpected: true }),
    sessionContext: async () => ({ requestHostnameContextPresent: false, organizationContextPresent: false, userContextPresent: false }),
    rlsMetadata: async () => ({ workspaceHostnamesRlsEnabled: true, workspaceHostnamesForceRls: true, organizationsRlsEnabled: true, organizationsForceRls: true }),
...
async function inspect(overrides: Partial<RuntimeDiagnosticProbeExecutor> = {}) {
...
describe('candidate tenant-router runtime diagnostic', () => {
  it('accepts one exact fixture hostname only', () => {
    expect(normalizedDiagnosticFixtureHostname('RC2-RELEASE-315960213001.next.labofscents.org.')).toBe(fixtureHostname)
  ...
  it('marks later probes incomplete after a database connection failure without exposing its cause', async () => {
    const diagnostic = await inspect({ databaseIdentity: async () => { throw new Error('database-name-or-secret') } })
  ...
  it('preserves independent evidence when execute is denied and resolver invocation throws', async () => {
    const diagnostic = await inspect({
      ...
      resolverInvocation: async () => { throw new Error('permission denied') },
    })
    expect(diagnostic).toMatchObject({
  ...
  it('distinguishes a direct RLS query returning false from a probe failure', async () => {
    const diagnostic = await inspect({ directHostnameVisibility: async () => false })
  ...
  it('continues safe probes when catalog metadata is unavailable', async () => {
    const diagnostic = await inspect({ resolverMetadata: async () => { throw new Error('catalog denied') } })
  ...
  it('returns an authenticated complete safe matrix even when an expected probe fails', async () => {
    const inspector = async () => inspect({ resolverPrivilege: async () => { throw new Error('hidden') } })
