...
function createPrismaRuntimeProbeExecutor(env: V2TenantRouterRuntimeDiagnosticEnv, hostname: string): RuntimeDiagnosticProbeExecutor {
  ...
  return {
    async databaseIdentity() {
      return firstRow(await prisma.$queryRaw<DatabaseIdentityRow[]>`
        SELECT
    ...
    async sessionContext() {
      return firstRow(await prisma.$queryRaw<SessionContextRow[]>`
        SELECT
    ...
    async rlsMetadata() {
      return firstRow(await prisma.$queryRaw<RlsMetadataRow[]>`
        SELECT
    ...
    async resolverMetadata() {
      return firstRow(await prisma.$queryRaw<ResolverMetadataRow[]>`
        SELECT
          EXISTS (
            SELECT 1
    ...
    async resolverPrivilege() {
      return firstRow(await prisma.$queryRaw<{ runtimeExecuteGranted: boolean }[]>`
    ...
    async directHostnameVisibility() {
      return firstRow(await prisma.$queryRaw<{ visible: boolean }[]>`
        SELECT EXISTS (
          SELECT 1
...
export async function inspectCandidateRuntime(
  ...
  try {
    try {
      ...
      diagnostic.hyperdriveProductionDatabaseMatch = equalSecret(await sha256(identity.databaseName), env.V2_EXPECTED_DATABASE_NAME_SHA.toLowerCase())
      diagnostic.runtimeCurrentUserMatchesExpected = identity.currentUserMatchesExpected
      diagnostic.runtimeSessionUserMatchesExpected = identity.sessionUserMatchesExpected
    } catch {
      return diagnostic
    }

    await applyProbe(() => executor.sessionContext(), (context) => {
      diagnostic.sessionContextProbeCompleted = true
      diagnostic.runtimeRequestHostnameContextPresent = context.requestHostnameContextPresent
      diagnostic.runtimeOrganizationContextPresent = context.organizationContextPresent
      diagnostic.runtimeUserContextPresent = context.userContextPresent
    })
    await applyProbe(() => executor.rlsMetadata(), (metadata) => {
      diagnostic.rlsMetadataProbeCompleted = true
      diagnostic.workspaceHostnamesRls = metadata.workspaceHostnamesRlsEnabled
      diagnostic.workspaceHostnamesForceRls = metadata.workspaceHostnamesForceRls
    ...
    await applyProbe(() => executor.resolverMetadata(), (metadata) => {
      ...
      diagnostic.functionOwnerOwnsWorkspaceHostnames = metadata.functionOwnerOwnsWorkspaceHostnames
      diagnostic.functionOwnerOwnsOrganizations = metadata.functionOwnerOwnsOrganizations
      ...
      diagnostic.functionOwnerForceRlsConstrained = metadata.functionOwnerForceRlsConstrained
    })
    await applyProbe(() => executor.resolverPrivilege(), (granted) => {
      diagnostic.resolverPrivilegeProbeCompleted = true
      diagnostic.runtimeExecuteGranted = granted
    })
    await applyProbe(() => executor.directHostnameVisibility(), (visible) => {
      diagnostic.directHostnameProbeCompleted = true
      diagnostic.runtimeDirectHostnameVisible = visible
    })
    await applyProbe(() => executor.directOrganizationVisibility(), (visible) => {
      diagnostic.directOrganizationProbeCompleted = true
      diagnostic.runtimeDirectOrganizationVisible = visible
    })
    await applyProbe(() => executor.resolverInvocation(), (visible) => {
...
export function runtimeDiagnosticExecutionPass(diagnostic: CandidateRuntimeDiagnostic) {
  return diagnostic.databaseProbeCompleted
