...
describe('safeScientificContainerError', () => {
  ...
  it('allows only known runtime errors from a controlled JSON protocol', async () => {
    ...
    await expect(safeScientificContainerError(new Response(JSON.stringify({ error: 'internal trace: secret=never-log' }), { status: 500 }))).resolves.toBe('SCIENTIFIC_CONTAINER_FAILED')
  })

  it('does not parse opaque non-JSON error bodies', async () => {
    await expect(safeScientificContainerError(new Response('container startup details', { status: 500 }))).resolves.toBe('SCIENTIFIC_CONTAINER_FAILED')
  })
})
