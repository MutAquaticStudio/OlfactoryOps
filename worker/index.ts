...
const routes: Route[] = [
  ...
  { method: 'POST', pattern: '/saas/custom-domains/provision', mutates: true, idempotent: true, rateLimit: sensitiveMutationRateLimit, writeGate: false, handler: ({ service, body, env }) => provisionCloudflareCustomDomain(service, body, env) },
  { method: 'GET', pattern: '/saas/custom-domains', handler: ({ service }) => service.customDomains() },
  { method: 'POST', pattern: '/saas/custom-domains/:id/refresh', mutates: true, idempotent: true, rateLimit: sensitiveMutationRateLimit, writeGate: false, handler: ({ service, params, env }) => refreshCloudflareCustomDomain(service, params.id, env) },
