...
- **Stripe Billing**: server-created Checkout and Customer Portal sessions, automatic tax enabled, and a signed webhook endpoint at `/api/v1/billing/stripe/webhook`. The Worker accepts only configured Price IDs for Artisan, Atelier, and Maison plans; subscription and invoice state changes only after the signed webhook is accepted.
- **Transactional email**: the in-app notification outbox delivers invite, new-device security, billing, and privacy-request messages through Resend when `RESEND_API_KEY` and `EMAIL_FROM` are configured. Business writes are never rolled back because a mail provider is unavailable.
- **Cloudflare for SaaS**: an Owner or Admin can call the custom-domain provisioning endpoint only after Cloudflare API token, SaaS zone ID, and optional custom origin are configured. The response contains DNS/DCV data for the customer instead of marking the hostname active prematurely.
- **Imports**: Material Library accepts CSV and XLSX, lets the user map columns, performs a dry-run with row-level errors, and commits only a validated, idempotent job. Lot import is also available through the API for an operations import client.
